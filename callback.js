...
async function getUsers(email, callback) {
    let userEmail = Users.find(search=> search.email === email)
    callback(userEmail)
}
async function getInfos(dataUser, callback) {
    let getZipCode = Infos.find(search => search.email === dataUser.email)
    callback(Object.assign(dataUser,getZipCode))    
}
async function getLocations(dataInfos, callback) {
    let dataUser =  Locations.find(search=> search.zipcode === dataInfos.zipcode)
    callback(Object.assign(dataInfos,dataUser))
}

for (let i = 0; i < Users.length; i++) {
    getUsers(Infos[i].email,sendEmail=>{
        getInfos(sendEmail,sendInfo=>{
            getLocations(sendInfo,sendData=>{
                dataBase.push(sendData)
            })
        })
    })
}
